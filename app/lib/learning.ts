...
export interface TechniqueGuide {
  ...
  /**
   * The condition that makes the fast route safe. A shortcut taught without the case it
   * breaks in is not a shortcut, it is a guess that usually works.
   */
  validity: string;
  traps: string[];
  keywords: string[];
...
export const TECHNIQUE_GUIDES: TechniqueGuide[] = [
  {
    ...
    validity: "Scale elimination decides an answer only when the options differ in order of magnitude; if two share a magnitude, finish the coefficient arithmetic. Squaring a proposed value is a valid check only when both sides are known to be non-negative.",
  ...
  },
  {
    ...
    validity: "Division by $3.6$ converts $\\text{km/h}$ to $\\text{m/s}$ in that direction only. A single length factor is valid for lengths alone: an area needs its square and a volume its cube.",
  ...
  },
  {
    ...
    validity: "Multipliers may be combined into one product only when each percentage acts on the result of the previous one. A change quoted against a different base has to be recalculated before it joins the chain.",
  ...
  },
  {
    ...
    validity: "Back-substitution is conclusive only against the original equation, and only when the options are exhaustive. Any step that multiplies through by an expression in $x$ can introduce roots the original never had.",
  ...
  },
  {
    ...
    validity: "A recognised triangle helps only once the right angle is established rather than assumed from the drawing. The $k$, $k^2$, $k^3$ scaling holds for genuinely similar figures — equal angles throughout, not merely a similar appearance.",
  ...
  },
  {
    ...
    validity: "Totals combine exactly, so a combined mean is safe whenever every group frequency is known. Anything built from grouped midpoints stays an estimate and cannot be reported as exact.",
  ...
  },
  {
    ...
    validity: "The complement rule holds for any well-defined event. Counting favourable over total selections needs every counted outcome to be equally likely, which fails without replacement unless the count itself tracks the shrinking pool.",
  ...
  },
  {
    ...
    validity: "The remainder theorem returns the remainder only for a linear divisor $x-a$. The discriminant classifies roots only for a genuine quadratic, so confirm the coefficient of $x^2$ cannot be zero.",
  ...
  },
  {
    ...
    validity: "$m_1m_2=-1$ requires both gradients to exist; a vertical and a horizontal line are perpendicular with no such product. The circle results need the coefficients of $x^2$ and $y^2$ to be equal and positive.",
  ...
  },
  {
    ...
    validity: "A stationary point can be called a maximum without a second test only for a quadratic with a negative $x^2$ coefficient. On a restricted interval the extreme value may sit at an endpoint, which a derivative never reports.",
  ...
  },
  {
    ...
    validity: "Every log law applies only where each argument is positive, so a solution that drives an argument to zero or below has to be discarded even when the algebra leading to it is sound.",
  ...
  },
  {
    ...
    validity: "A change inside the function acts horizontally and in the opposite sense; a change outside acts vertically in the stated sense. Combined transformations are reliable only when applied in the order the expression is built up.",
  ...
  },
  {
    ...
    validity: "The symmetry shortcuts need the function to be genuinely odd or even and the interval centred on zero. A signed integral equals a geometric area only where the curve stays on one side of the axis throughout.",
  ...
  },
  {
    ...
    validity: "The sum to infinity exists only when $|r|<1$, so verify the ratio before quoting it. Term and sum formulae also depend on whether the first term is indexed from $0$ or from $1$.",
  ...
  },
  {
    ...
    validity: "A reference angle supplies one solution; the interval and the quadrant signs decide which others belong with it. Squaring or dividing through by a trigonometric term can add or lose solutions, so test each against the original equation.",
  ...
  },
  {
    ...
    validity: "Series and parallel reduction assumes ideal connecting wires and negligible internal resistance unless the question says otherwise. Every power form is exact, but only once that branch's own current and potential difference are known.",
  ...
  },
  {
    ...
    validity: "The turns ratio and equal input and output power may be used only when the transformer is stated to be ideal or no losses are given. The direction rules assume the field, current and motion are mutually perpendicular.",
  ...
  },
  {
    ...
    validity: "A definition-led result holds at any scale provided every quantity shares one unit system. Hooke's law applies only below the limit of proportionality.",
  ...
  },
  {
    ...
    validity: "Mechanical energy is conserved only when no dissipative work is described. Momentum is conserved for the whole system whenever external forces are negligible during the interaction. Constant velocity means zero resultant force, not zero force.",
  ...
  },
  {
    ...
    validity: "The factor $(1/2)^n$ is exact for any $n$, whole or not, provided the half-life is constant. The balancing rules describe the nucleus alone: an emitted beta particle carries no nucleon number.",
  ...
  },
  {
    ...
    validity: "$E=mc\\Delta T$ applies only while the substance remains in one state; the flat section of a heating curve needs $E=mL$ instead. Gas proportionality requires a fixed mass of gas and temperature in kelvin.",
  ...
  },
  {
    ...
    validity: "Frequency is fixed by the source, so the speed-and-wavelength ratio holds at any boundary between media. Halving the measured distance applies to an echo only when the reflection returns along the same path.",
